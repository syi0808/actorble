...
describe('builder authoring session', () => {
    ...
    for (const { family, expected, issuePath } of cases) {
      ...
      if (issuePath === undefined) {
        ...
        expect(validation).toMatchObject({
          ok: false,
          issues: expect.arrayContaining([expect.objectContaining({ path: issuePath })]),
        });
      }
    }
  });
  ...
  it('updates step fields, writes selected target slots, and validates to a portable scenario document', () => {
    ...
    const selectedSlot = unwrap(
      selectTargetSlot(inserted, {
        kind: 'step-target',
        stepId: 'step-1',
      }),
    );
    const assigned = unwrap(
      assignLocatorToSelectedTargetSlot(selectedSlot, {
        strategy: 'testId',
        value: 'email-input',
      }),
    );
    const updated = unwrap(
      updateStepFields(assigned, 'step-1', {
  ...
  it('supports text selection endpoint target slots without storing UI state', () => {
    ...
    const ranged = unwrap(
      updateStepFields(created, 'selection-step', {
        target: {
          anchor: {
            target: emptyTarget(),
            offset: 2,
    ...
    expect(
      listTargetSlotsForStep(ranged.draftDocument?.steps[0] ?? neverStep(), 'selection-step').map(
        (slot) => slot.kind,
      ),
    ).toEqual(['selection-anchor', 'selection-focus']);

    const withAnchor = unwrap(
      assignLocatorToTargetSlot(
        ranged,
        {
    ...
    expect(JSON.stringify(withFocus.draftDocument)).not.toContain(
      'selection-anchor:selection-step',
    );
  });
