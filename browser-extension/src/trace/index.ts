export type RuntimeRunStatus = 'idle' | 'running' | 'paused' | 'stopped' | 'completed' | 'failed';
...
export function createTraceDisplayStore(options: TraceDisplayStoreOptions = {}): TraceDisplayStore {
  const historyLimit = positiveIntegerOrDefault(options.historyLimit, DEFAULT_TRACE_HISTORY_LIMIT);
  ...
  function getOrCreateRun(runId: string, create: () => MutableTraceRunGroup): MutableTraceRunGroup {
...
export function createDevtoolsTracePanelStore(
  ...
  function getSnapshot(): DevtoolsTracePanelSnapshot {
    ...
    const resolvedSelectedRunId = selectedRunId ?? state.currentRunId ?? state.runs.at(-1)?.runId;
    const runs = state.runs.map((run) => devtoolsRunView(run, run.runId === resolvedSelectedRunId));
...
function devtoolsRunView(run: TraceRunDisplayView, selected: boolean): DevtoolsTracePanelRunView {
...
function traceSummaryFor(debugSnapshot: RuntimeDebugSnapshot | undefined): DevtoolsTraceSummary {
...
function surfaceRowFromDisplayEvent(event: TraceDisplayEvent): DevtoolsFrameSurfaceRow | undefined {
...
function latestErrorEvent(events: readonly TraceDisplayEvent[]): TraceDisplayEvent | undefined {
