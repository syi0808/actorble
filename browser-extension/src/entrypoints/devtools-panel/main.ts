...
function render(): void {
  ...
  renderDetailList(locatorList, selectedRun?.locatorDiagnostics ?? [], locatorRow, 'No locator diagnostics')
  renderDetailList(spanList, selectedRun?.debugSnapshot?.trace.spans ?? [], spanRow, 'No spans')
  renderDetailList(eventList, selectedRun?.debugSnapshot?.trace.events ?? [], eventRow, 'No trace events')
  renderDetailList(
    snapshotList,
  ...
  traceJson.textContent = selectedRun?.debugSnapshot === undefined
    ? 'No debug snapshot'
...
function spanRow(
  span: RuntimeTraceSpanSnapshot,
): Readonly<{ title: string; detail: string }> {
  const duration = span.endedAt === undefined ? 'open' : `${Math.round(span.endedAt - span.startedAt)}ms`
...
function warningRow(
  ...
  return {
    ...
    detail: warning.details === undefined ? `${Math.round(warning.at)}` : compactJson(warning.details),
  }
}
