...
export type BuilderRunStatus = 'running' | 'paused' | 'completed' | 'failed' | 'stopped';
...
export type BuilderRecordStatus = 'recording' | 'stopped' | 'failed';
...
export function updateDocumentFields(
  ...
  return ok(withDraftDocument(state, next as BuilderDraftDocument, state.selectedStepId, true));
}
...
export function addStep(
  ...
  return ok(
    withDraftDocument(state, documentWithStep, stepIdFor(step, document.steps.length), true),
  );
}
...
export function deleteStep(
  ...
  const nextSelectedStepId =
    nextSelectedStep === undefined ? undefined : stepIdFor(nextSelectedStep, nextSelectedIndex);
...
export function reorderStep(
  ...
  const steps = [...remaining.slice(0, insertionIndex), step, ...remaining.slice(insertionIndex)];
    ...
    withDraftDocument(
      {
        ...state,
        selectedTargetSlot:
          slotReferencesStep(state.selectedTargetSlot, stepId) &&
...
export function listTargetSlotsForStep(
  ...
  switch (step.action) {
    ...
    case 'waitFor':
      return isTargetWaitCondition(step.input) ? [{ kind: 'waitFor-target', stepId }] : [];
    case 'reveal':
...
function emptySession(
  ...
  return {
    ...
    dirty: false,
    issues: [],
    ...(options.createScenarioId === undefined
      ? {}
      : { createScenarioId: options.createScenarioId }),
...
> {
  ...
  const index = document.steps.findIndex(
    (step, stepIndex) => stepIdFor(step, stepIndex) === stepId,
  );
  if (index < 0) {
    return failure({
      code: 'invalid_document',
...
function applyNullable(target: Record<string, unknown>, key: string, value: unknown): void {
  if (value === undefined) {
    return;
  }
...
function applyDeletable(target: Record<string, unknown>, key: string, value: unknown): void {
  if (value === undefined) {
    return;
  }
...
function slotReferencesStep(slot: BuilderTargetSlot | undefined, stepId: string): boolean {
...
function slotCanWrite(step: BuilderDraftStep, slot: BuilderTargetSlot | undefined): boolean {
  ...
  switch (slot.kind) {
    case 'step-target':
      return (
        ['click', 'moveTo', 'doubleClick', 'focus', 'typeInto', 'fill'].includes(step.action) ||
...
function isTextSelectionEndpointTarget(value: unknown): value is Readonly<{
