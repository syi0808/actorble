...
export function createTargetPickerView(snapshot: TargetPickerSnapshot): TargetPickerView {
...
export function normalizePickedTargetMetadata(
  ...
  return {
    ...
    ...(target.documentOrderIndex === undefined
      ? {}
      : { documentOrderIndex: target.documentOrderIndex }),
    ...
    ...(target.labelText === undefined
      ? {}
      : { labelText: compactText(target.labelText, TEXT_SUMMARY_LIMIT) }),
...
function sessionFromReceipt(fallback: TargetPickerSession, value: unknown): TargetPickerSession {
  if (!isRecord(value)) {
    return fallback;
  }
  ...
  return {
    ...
    ...(typeof value.scenarioId === 'string'
      ...
      : optionalScenarioId(fallback.scenarioId)),
...
function normalizeStartInput(
  input: string | TargetPickerStartInput | undefined,
): TargetPickerStartInput {
