...
export function normalizeRecordedEvents(
  ...
  function flushPendingText(): ExtensionResult<void> {
    ...
    const result = buildTextStep(pendingText.event, pendingText.index, nextStepId(records));
    if (!result.ok) {
      return result;
    }
...
function buildTextStep(
  ...
  const action: 'fill' | 'typeInto' = isFillTarget(event.target) ? 'fill' : 'typeInto';
...
function pointerButtonName(button: number): ScenarioActionOptions['button'] | undefined {
  switch (button) {
    case 0:
      return 'primary';
    case 1:
...
function sensitiveNote(event: RawRecordedTextEvent): string {
  const valueHandling =
    event.value === RECORDER_MASKED_VALUE
      ? 'masked'
      : event.value.length === 0
        ? 'omitted'
        : 'recorded';
  const reason = event.sensitiveReason === undefined ? '' : ` (${event.sensitiveReason})`;
...
function targetDescription(target: RecorderTargetSnapshot): string {
  const tagName = target.tagName.toLowerCase();
  const handle = compactText(target.id) === undefined ? tagName : `${tagName}#${target.id}`;
  const name = compactText(
    target.ariaLabel ?? target.labelText ?? target.placeholder ?? target.text ?? target.name,
  );
...
function selectionTarget(event: RawRecordedSelectionEvent): RecorderTargetSnapshot | undefined {
