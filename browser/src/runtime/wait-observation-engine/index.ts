...
export class BrowserWaitObservationEngine implements WaitObservationEngine {
  ...
  constructor(options: WaitObservationEngineOptions = {}) {
    ...
    const layoutInvalidation =
      options.layoutInvalidation ??
      new BrowserLayoutInvalidationTracker({
        dom: this.#dom,
  ...
  async waitFor(condition: WaitCondition, options: WaitOptions = {}): Promise<WaitResult> {
    ...
    try {
      const result = await this.#withTimeout(operation, options, details, (signal) =>
        this.#waitForTrackedCondition(condition, signal, diagnostics, []),
      );
  ...
  async #waitForCompositeCondition(
    ...
    const tasks = condition.conditions.map((child, index) =>
      this.#waitForTrackedCondition(child, branches[index].controller.signal, diagnostics, [
        ...path,
        index,
      ]),
    );
  ...
  async #waitForTargetTextCondition(
    ...
    for (;;) {
      ...
      try {
        ...
        const observation = observationFromContentTargetError(
          error,
          target,
          summarizeMatcher(condition.value),
        );
  ...
  async #resolveTarget(
    ...
    const handle =
      cachedHandle ??
      (isTargetHandle(target) ? target : await this.#resolver.resolve(toLocator(target), {}));
...
function staleTargetNoLongerResolves(error: ActorbleError): boolean {
  ...
  return (
    cause instanceof ActorbleError &&
...
function collectPendingChildren(
  ...
  if (condition.kind === 'all' || condition.kind === 'any') {
    if (condition.conditions.length === 0) {
      pending.set(compositePathKey(path), {
        path: [...path],
        condition: summarizeCondition(condition),
      });
    } else {
...
function waitForCancellation(signal: WaitOptions['signal']): Promise<never> {
  ...
  return new Promise((_, reject) => {
    signal?.addEventListener('abort', () => reject(cancellationError('wait.for', signal.reason)), {
      once: true,
    });
  });
}

function isTargetWaitCondition(condition: WaitCondition): condition is TargetWaitCondition {
  return (
