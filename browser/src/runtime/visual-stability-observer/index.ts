...
export class BrowserVisualStabilityObserver implements VisualStabilityObserver {
  ...
  constructor(options: VisualStabilityObserverOptions) {
    ...
    this.#scrollChain =
      options.scrollChain ?? new ActorbleScroller2ScrollChainResolver(new BrowserDomAdapter());
      ...
      for (;;) {
        ...
        for (const state of scrollStates) {
          ...
          if (
            ...
            scrollChanged(state.previous, current, policy.threshold)
          ) {
        ...
        const observedStableFrames =
          target === undefined ? minimumScrollStableFrames(scrollStates) : geometryStableFrames;
        ...
        const geometryStable = target === undefined || geometryStableFrames >= policy.stableFrames;
...
function scrollChanged(
  ...
  return (
    Math.abs(previous.scrollLeft - current.scrollLeft) > threshold ||
...
function minimumScrollStableFrames(states: readonly ScrollState[]): number {
  return states.length === 0
    ...
    : Math.min(...states.map((state) => state.stableFrames));
}
