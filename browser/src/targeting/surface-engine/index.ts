...
export class BrowserSurfaceEngine implements SurfaceEngine {
  ...
  async #executeAdjustedReveal(
    ...
    const selected =
      (options.container ?? 'all') === 'nearest' ? initial.steps.slice(0, 1) : initial.steps;
    const offset = options.offset ?? { x: 0, y: 0 };
    const executed: ExecutedScrollStep[] = [];

    for (const step of selected) {
      executed.push(
        await this.#scroller().to(
          step.surface,
  ...
  #readViewportScroll(): Point {
    return this.#platform.readScroll(this.#dom.getViewportScrollTarget(this.#dom.getRoot()));
  }
