...
export class BrowserDomAdapter implements DomAdapter {
  ...
  getActiveAnimationCount(element?: Element): number {
    ...
    const getAnimations =
      'getAnimations' in root && typeof root.getAnimations === 'function'
    ...
    const animations = element === undefined ? getAnimations() : getAnimations({ subtree: true });

    return animations.filter((animation) => animation.playState === 'running' || animation.pending)
      .length;
  }
...
function subscribeToUrlChanges(ownerWindow: Window, listener: ActorbleListener<void>): Disposable {
