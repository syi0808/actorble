...
import type {
  ...
  TraceCollector,
  TraceEvent,
} from '../../diagnostics/diagnostics-trace/index.js';
...
export class Actorble {
  ...
  constructor(readonly options: ActorbleFacadeOptions = {}) {
    ...
    const resolver = options.resolver ?? new BrowserTargetResolver({ dom, trace, clock: timeline });
...
function rootForDomAdapter(root: ActorbleOptions['root']): Document | ShadowRoot | undefined {
  if (root === undefined) {
    return undefined;
  }
