...
import type { Point, Rect, TargetHandle, VisualTextVisibility } from '../../shared/index.js';
...
export class BrowserVisualLayer implements VisualLayer {
  ...
  showKeystroke(request: KeystrokeVisualRequest): void {
    ...
    const visibility = request.textVisibility ?? this.options.textVisibility ?? 'plain';
...
const DEFAULT_CURSOR_ARROW_PATH =
  'M 2,2 L 18.4,17.6 L 12,18.8 L 16.8,27.4 L 12.3,29.4 ' + 'L 7.2,20.4 L 2,25.8 Z';
...
const NOT_ALLOWED_CURSOR_PATH =
  'M 11,2.5 A 8.5,8.5 0 1 0 11,19.5 A 8.5,8.5 0 1 0 11,2.5 ' + 'M 5.2,16.8 L 16.8,5.2';
...
const WAIT_CURSOR_SAND_PATH = 'M 8.2,6 L 13.8,6 L 11,8.4 Z M 8.2,16 L 13.8,16 L 11,13.6 Z';

const PROGRESS_CURSOR_SPINNER_PATH =
  'M 25.5,8 A 4.5,4.5 0 1 1 22.2,3.7 M 22.2,3.7 L 25,3.3 ' + 'M 22.2,3.7 L 23.3,6.3';
...
function scaleCursorVisualSpec(spec: CursorVisualSpec, scale: number): CursorVisualSpec {
  if (scale === 1) {
    return spec;
  }
...
function applyCursorElementStyle(cursor: HTMLElement, spec: CursorVisualSpec, point: Point): void {
...
function createCursorSvg(
  ...
  hotspotShift.setAttribute('transform', `translate(${-options.hotspot.x} ${-options.hotspot.y})`);
...
function safeLabelForTarget(target: TargetHandle | undefined): string {
  ...
  return target.debug.name ?? target.debug.description ?? target.debug.selector ?? target.id;
}
