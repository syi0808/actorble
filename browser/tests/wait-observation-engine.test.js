...
function createObservationPorts(target, options = {}) {
  ...
  return {
    ...
    interactability: {
      inspect: vi.fn(
        async () => reports.shift() ?? reports.at(-1) ?? interactabilityReportFor(target),
      ),
      canClick: vi.fn(),
      canFocus: vi.fn(),
      canType: vi.fn(),
...
describe('BrowserWaitObservationEngine', () => {
        ...
        (_strategy, options) =>
          new Promise((_, reject) => {
            ...
            options.signal.addEventListener(
              'abort',
              () => reject(actorbleError('ACTION_CANCELLED', 'cancelled')),
              { once: true },
            );
          }),
      ),
  ...
  it('reports target-scoped text waits as an explicit extension point', async () => {
    ...
    await expect(
      engine.waitFor(text('Saved successfully', { target: css('#status') })),
    ).resolves.toMatchObject({ satisfied: true });
    await expect(
      engine.waitFor(text(/^Saved successfully$/g, { target: css('#status') })),
    ).resolves.toMatchObject({ satisfied: true });
    expect(timeline.settle).toHaveBeenCalledOnce();
  });
  ...
  it('matches target text exactly while preserving root text containment', async () => {
    ...
    await expect(rootEngine.waitFor(text('Project created'))).resolves.toMatchObject({
      satisfied: true,
    });
    ...
    await expect(
      targetEngine.waitFor(text('Project created', { target: css('#status') })),
    ).resolves.toMatchObject({ satisfied: true });
    expect(timeline.settle).toHaveBeenCalledOnce();
  });
  ...
  it('waits for input and select values and rejects unsupported value targets', async () => {
    ...
    await expect(engine.waitFor(value(css('#name'), /^ready$/g))).resolves.toMatchObject({
      satisfied: true,
    });
    document.querySelector('#state').value = 'ready';
    await expect(engine.waitFor(value(css('#state'), 'ready'))).resolves.toMatchObject({
      satisfied: true,
    });
    await expect(engine.waitFor(value(css('#notes'), 'draft'))).resolves.toMatchObject({
      satisfied: true,
    });
  ...
  it('distinguishes absent attributes from empty values and supports regular expressions', async () => {
    ...
    await expect(
      engine.waitFor(attribute(css('#panel'), 'data-state', null)),
    ).resolves.toMatchObject({ satisfied: true });
    panel.setAttribute('data-state', '');
    await expect(engine.waitFor(attribute(css('#panel'), 'data-state', ''))).resolves.toMatchObject(
      { satisfied: true },
    );
    panel.setAttribute('data-state', 'ready');
    await expect(
      engine.waitFor(attribute(css('#panel'), 'data-state', /^ready$/g)),
    ).resolves.toMatchObject({ satisfied: true });
    ...
    await expect(
      transitionEngine.waitFor(attribute(css('#panel'), 'data-state', 'ready')),
    ).resolves.toMatchObject({ satisfied: true });
  ...
  it('matches root-relative, absolute, and RegExp URLs across SPA transitions', async () => {
    ...
    await expect(engine.waitFor(url('/projects/1?tab=summary#details'))).resolves.toMatchObject({
      satisfied: true,
    });
    await expect(
      engine.waitFor(url(new URL('/projects/1?tab=summary#details', location.href).href)),
    ).resolves.toMatchObject({ satisfied: true });
    await expect(
      engine.waitFor(url(/\/projects\/1\?tab=summary#details$/g)),
    ).resolves.toMatchObject({ satisfied: true });
  });
  ...
  it('disposes URL observation when an any sibling wins and supports the next wait', async () => {
    ...
    await expect(
      engine.waitFor(any(url('/never'), { kind: 'custom', predicate: () => true })),
    ).resolves.toMatchObject({ satisfied: true });
    expect(dispose).toHaveBeenCalledOnce();
  ...
  it('redacts matcher, observed content, locator text, and URL components from diagnostics', async () => {
    ...
    const timeline = createTimeline({ settle: vi.fn(() => new Promise(() => {})) });
    const engine = new BrowserWaitObservationEngine({ timeline, trace });
    const promise = engine.waitFor(value(css('#locator-secret'), 'matcher-secret'), {
      timeout: 25,
    });
    const failurePromise = promise.catch((error) => error);
    await vi.advanceTimersByTimeAsync(25);
  ...
  it('reuses root text observations across unchanged retries and refreshes after mutation', async () => {
    ...
    await expect(engine.waitFor({ kind: 'text', value: 'Project created' })).resolves.toMatchObject(
      {
        satisfied: true,
        strategy: 'interaction-stable',
      },
  ...
  it.each([
    ...
    'resolves %s after its target observation transitions',
    async (_kind, createCondition, reportEnabled) => {
      document.body.innerHTML = '<button id="save">Save</button>';
      const target = targetHandle(document.querySelector('#save'));
      let attempt = 0;
      const ports = createObservationPorts(target, {
      ...
      await expect(
        engine.waitFor(createCondition(css('#save')), { timeout: 100 }),
  ...
  it('resolves detached when a locator no longer resolves without geometry reads', async () => {
    ...
    const ports = createObservationPorts(target, {
      resolver: {
        resolve: vi.fn(async () => {
          throw actorbleError('TARGET_NOT_FOUND', 'gone');
        }),
      },
    });
    const engine = new BrowserWaitObservationEngine({ timeline: createTimeline(), ...ports });

    await expect(engine.waitFor(detached(css('#toast')))).resolves.toMatchObject({
      satisfied: true,
    });
    expect(ports.geometry.snapshot).not.toHaveBeenCalled();
    expect(ports.interactability.inspect).not.toHaveBeenCalled();
  ...
  it('treats failed locator-backed stale recovery as detached but preserves structural errors', async () => {
    ...
    await expect(
      new BrowserWaitObservationEngine({ timeline: createTimeline(), ...detachedPorts }).waitFor(
        detached(stale),
      ),
    ).resolves.toMatchObject({ satisfied: true });
    ...
    await expect(
      new BrowserWaitObservationEngine({ timeline: createTimeline(), ...structuralPorts }).waitFor(
        detached(stale),
      ),
  ...
  it('records condition-specific timeout context and cancels target waits cleanly', async () => {
    ...
    controller.abort('stopped');
    await expect(
      engine.waitFor(attached(css('#save')), { signal: controller.signal }),
    ).rejects.toMatchObject({
      code: 'ACTION_CANCELLED',
