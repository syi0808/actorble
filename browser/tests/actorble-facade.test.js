...
describe('Actorble facade', () => {
  ...
  it('delegates public entrypoints to injected modules', async () => {
    ...
    await expect(actorble.reveal(locator, revealOptions)).resolves.toMatchObject({
      fullyVisible: true,
    });
    await expect(actorble.scrollTo(scrollPosition, scrollOptions)).resolves.toMatchObject({
      changed: true,
    });
    await expect(actorble.scrollBy(scrollDelta, scrollOptions)).resolves.toMatchObject({
      changed: true,
    });
    ...
    expect(orchestrator.focus).toHaveBeenCalledWith(locator, {
      ...targetLifecycleDefaults,
      ...focusOptions,
    });
    expect(orchestrator.type).toHaveBeenCalledWith('hello', {
      ...actionWaitDefaults,
      ...typeOptions,
    });
    ...
    expect(orchestrator.fill).toHaveBeenCalledWith(locator, 'filled', {
      ...targetLifecycleDefaults,
      ...fillOptions,
    });
    expect(orchestrator.press).toHaveBeenCalledWith('Shift+K', {
      ...actionWaitDefaults,
      ...pressOptions,
    });
    ...
    expect(orchestrator.pointerSequence).toHaveBeenCalledWith(pointerSequence, {
      ...actionWaitDefaults,
  ...
  it('creates a default module graph that can resolve and click through the facade', async () => {
    ...
    expect(actorble.getTrace().spans).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'action.click', status: 'ok' })]),
    );
  });
  ...
  it('creates a default module graph that can click the current pointer target', async () => {
    ...
    await expect(actorble.clickCurrent({ duration: 0, pressDwell: 0 })).resolves.toBeUndefined();
  ...
  it('creates a default module graph that can focus through the facade', async () => {
    ...
    expect(actorble.getTrace().spans).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'action.focus', status: 'ok' })]),
    );
  });
  ...
  it('creates a default module graph that can type into the current focus', async () => {
    ...
    expect(actorble.getTrace().spans).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'action.type', status: 'ok' })]),
    );
  });
  ...
  it('creates a default module graph that can press keys on the current focus', async () => {
    ...
    expect(actorble.getTrace().spans).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'action.press', status: 'ok' })]),
    );
  });
  ...
  it('creates a default module graph that can fill a target value', async () => {
    ...
    expect(actorble.getTrace().spans).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'action.fill', status: 'ok' })]),
    );
  });
  ...
  it('creates a default module graph that can scroll to an explicit position', async () => {
    ...
    expect(actorble.getTrace().spans).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'action.scrollTo', status: 'ok' })]),
    );
  ...
  it('creates a default module graph that can drag through the facade', async () => {
    ...
    expect(actorble.getTrace().spans).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'action.drag', status: 'ok' })]),
    );
    expect(actorble.getTrace().events).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
