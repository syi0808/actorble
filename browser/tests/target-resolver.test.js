...
} from '../src/targeting/target-resolver/index.js';
...
describe('BrowserTargetResolver', () => {
  ...
  it('uses matchIndex to select a specific query locator match', async () => {
    ...
    await expect(
      resolver.resolve(css('.choice', { matchIndex: 1 }), { strict: true }),
    ...
    await expect(resolver.exists(role('button', { name: /./, matchIndex: 3 }))).resolves.toBe(
      false,
    );
  ...
  it('limits resolver read memoization to a single resolve call', async () => {
    ...
    await expect(
      resolver.resolve(role('button', { name: 'Publish draft' })),
    ).resolves.toMatchObject({
      element: button,
  ...
  it('keeps diagnostics candidate snapshot shape while reusing per-pass debug reads', async () => {
    ...
    await expect(
      resolver.resolve(role('button', { name: 'Save' }), { strict: true }),
  ...
  it('resolves role locators by accessible name ranking and strict ambiguity', async () => {
    ...
    await expect(
      resolver.resolve(role('button', { name: 'Create Project' })),
    ).resolves.toMatchObject({
      element: document.querySelector('#create'),
    });
    await expect(resolver.resolveAll(role('button', { name: /create/i }))).resolves.toMatchObject([
      { element: document.querySelector('#create') },
  ...
  it('resolves role locators by native label accessible names', async () => {
    ...
    await expect(
      resolver.resolve(role('textbox', { name: 'ARIA Labelled' })),
    ).resolves.toMatchObject({
      element: document.querySelector('#labelled'),
    });
    await expect(
      resolver.resolve(role('textbox', { name: 'Native Labelled' })),
    ).rejects.toMatchObject({
      code: 'TARGET_NOT_FOUND',
    });
    ...
    await expect(
      resolver.resolve(role('textbox', { name: 'Hidden Control' })),
    ).rejects.toMatchObject({
      code: 'TARGET_NOT_FOUND',
    });
  });
  ...
  it('prunes large nested text matches without pairwise contains reads', async () => {
    ...
    expect(all.map((handle) => handle.element)).toEqual(
      Array.from({ length: rowCount }, (_, index) =>
        document.querySelector(`#nested-leaf-${index}`),
      ),
    );
