...
describe('BrowserSurfaceEngine scroller2 integration', () => {
  ...
  it('delegates reveal and maps scroller2 results to Actorble contracts', async () => {
    ...
    expect(scrollEngine.reveal).toHaveBeenCalledWith(target, expect.objectContaining({
      visibility: 'full',
      block: 'center',
      motion: 'instant',
      settle: false,
  ...
  it('delegates absolute and relative viewport scrolling', async () => {
    ...
    expect(scrollEngine.to).toHaveBeenCalledWith(window, { x: 10, y: 20 }, expect.objectContaining({
      motion: 'instant',
      settle: false,
    }))
  ...
  it('maps timed motion and scroll-stable settlement options', async () => {
    ...
    expect(scrollEngine.to).toHaveBeenCalledWith(window, { x: 10, y: 20 }, expect.objectContaining({
      motion: expect.objectContaining({ type: 'tween', duration: 120, easing: expect.any(Function) }),
